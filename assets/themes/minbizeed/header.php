...
<html <?php language_attributes(); ?>>
    ...
    <head>
        ...
        <script async src="https://www.googletagmanager.com/gtag/js?id=UA-111460951-1"></script>
        <script>
...
<?php
global $wpdb;
$realtime_notifications     = "select * from " . $wpdb->prefix . "realtime_notifications order by id desc";
$realtime_notifications_row = $wpdb->get_results( $realtime_notifications );
?>
    <div class="ticker-wrap" style="display: none">
        <div class="ticker">
			<?php
			foreach ( $realtime_notifications_row as $realtime_notifications_r ) {
				$notifTitle     = $realtime_notifications_r->title;
				$notifTimestamp = $realtime_notifications_r->date_time;

				$notifDateTimeObj = new DateTime( "@" . $notifTimestamp );
				$notifTimeUTC     = $notifDateTimeObj->format( 'g:i a' );

				$notifDateTimeObj->setTimezone( new DateTimeZone( 'Asia/Beirut' ) );

				$notifTimeLb = $notifDateTimeObj->format( 'g:i a' );

				if ( date( 'Ymd' ) == date( 'Ymd', $notifTimestamp ) ) {
					?>
                    <div class="ticker__item">
						<?php echo '<span class="date" data-utcTime="' . $notifTimeUTC . '">' . $notifTimeLb . '</span>' . $notifTitle . '<span class="pointless">●</span>'; ?>
                    </div>
                    <script type="text/javascript">
                        jQuery('.ticker-wrap').css('display', 'block');
                    </script>
					<?php
				}
			}

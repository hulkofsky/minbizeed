...
var mongoDB = 'mongodb://127.0.0.1/'+config.MONGODB;
...
var PennyBids = mongoose.model('PennyBids', schema );
...
module.exports = {
    ...
    getPriceData: function ($id) {
        var query = squel
            ...
            .where(['post_id = ', $id, ' AND meta_key IN (\'start_price\', \'current_bid\', \'minimum_price\')'].join(""))
            .order("FIELD(meta_key, 'start_price', 'current_bid', 'minimum_price')")
            .toString();
    ...
    getAdminAutoBidUsers: function ($id) {
        var query = squel
            .select('meta_value')
            .from(table_prefix+'postmeta')
            .where(['post_id = ', $id, ' AND meta_key LIKE \'admin_auto_bid_users\''].join(""))
            .toString();
    ...
    setNoWinnerOnAuction: function ($id) {
        ...
        q.allSettled([
            ...
            /* MongoDB Changes */
            PennyBids.findOneAndUpdate({pid:$id},{ $set: { winner : 0}},{ 'sort': { date_made : -1 }},function(err,results){ var deferred = q.defer(); deferred.resolve(results); return deferred.promise; })
            /* MongoDB Changes */,
    ...
    setAuctionClosed: function ($id) { // sets an auction status
        ...
        q.allSettled([this.query(closeQuery),
            ...
            q.allSettled([
                ...
                /* MongoDB Changes */
                PennyBids.findOneAndUpdate({pid:$id,uid:currentBidder},{ $set: { "winner" : 1}},{ 'sort': { date_made : -1,bid:-1 },$limit:1}).exec(function(err,results){ var deferred = q.defer(); deferred.resolve(results); return deferred.promise; })
                /* MongoDB Changes */
    ...
    getAuctionBids: function ($id) {
        ...
        /* MongoDB Changes */
        return PennyBids.find({pid:$id}).exec(function(err,results){ var deferred = q.defer(); deferred.resolve(results); return deferred.promise; });
        /* MongoDB Changes */
    ...
    insertBid: function ($id, $userid, $price) {
        ...
        /* MongoDB Changes */
        var pennyBidsQuery = new PennyBids({
                uid: $userid,
                pid: $id,
                bid: $price,
                paid: 0,
        ...
        pennyBidsQuery.save(function (err) {
    ...
    getLastBid: function ($id) {
        ...
        /* MongoDB Changes */
        return PennyBids.find({pid:$id}).sort({ bid: -1 }).exec(function(err,results){ var deferred = q.defer(); deferred.resolve(results); return deferred.promise; });
        /* MongoDB Changes */
    ...
    getCurrentBidder: function ($auction) {
        ...
        //return this.query(query)
        /* MongoDB Changes */
        return PennyBids.find({pid:$auction}).sort({ bid: -1 }).limit(1).exec(function(err,results){ var deferred = q.defer(); deferred.resolve(results); return deferred.promise; });
        /* MongoDB Changes */
    },
    ...
    getAuctionParticipants: function ($auction) {
        ...
        /* MongoDB Changes */
        return PennyBids.find({pid:$auction}).select({ uid:1 }).exec(function(err,results){ var deferred = q.defer(); deferred.resolve(results); return deferred.promise; });
        /* MongoDB Changes */
    },

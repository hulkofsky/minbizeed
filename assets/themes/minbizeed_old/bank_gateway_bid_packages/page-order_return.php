<?php
/**
 * Bank gateway bids payment return
 *
 * @package  MinBizeed
 * @company  Triangle Mena <http://trianglemena.com>
 * @developer  Maroun Melhem <http://maroun.me>
 *
 * Template: Payment return order Form
 * Template Name: Payment return order Form template
 */
...
if (is_user_logged_in()) {
    ...
    <div class="buy_bids_page">
        ...
        <div class="main-container" role="main">
            ...
            /*Getting UserID and Bid Package start*/
            $s = "select * from `tb_5550121_migs_transactions_data` where `vpc_MerchTxnRef`='$merchTxnRef' AND `verified`=1";
            $r = $wpdb->get_results($s);
            if ($r) {
                foreach ($r as $row) {
                    $user_id = $row->user_id;
                    $bid_package = $row->bid_package;
                }
            }
            /*Getting UserID and Bid Package end*/
            ...
            $transaction_exist_checker = "select * from `tb_5550121_penny_payment_transactions` where `uid2`='$merchTxnRef'";
            ...
            if (empty($transaction_exist_checker_r)) {
                ...
                $insite_trans = $wpdb->insert(
                    'tb_5550121_penny_payment_transactions',
                    array(
                        'uid' => $user_id,
                        'datemade' => "$date_time",
                        'amount' => "$amount",
                        'tp' => "Bid Package= $bid_package",
                        'uid2' => $merchTxnRef,
                    ),
                    array(
                        '%s',
            ...
            switch ($txnResponseCode) {
                case "0" :
                    $case = "0";
                    /*Getting User balance start*/
                    $user_bids_tot = "select * from `tb_5550121_usermeta` where `meta_key`='user_credits' AND `user_id`=$user_id";
                    $r_user_bids_tot = $wpdb->get_results($user_bids_tot);
                    if ($r_user_bids_tot) {
                        foreach ($r_user_bids_tot as $row_r_user_bids_tot) {
                            $user_balance = $row_r_user_bids_tot->meta_value;
                        }
                    }
                    /*Getting User balance end*/

                    /*Getting Bid package bids start*/
                    $bid_package_tot = "select * from `tb_5550121_penny_packages` where `id`='$bid_package'";
                    $r_bid_packages_tot = $wpdb->get_results($bid_package_tot);
                    if ($r_bid_packages_tot) {
                        foreach ($r_bid_packages_tot as $row_r_bid_packages_tot) {
                            $package_bid = $row_r_bid_packages_tot->bids;
                        }
                    }
                    /*Getting Bid package bids end*/

                    /*checking if bids were added in this transaction start*/
                    $bid_added_checker = "select * from `tb_5550121_penny_payment_transactions` where `uid2`='$merchTxnRef'";
                    ...
                    if ($is_transmitted == "NO") {
                        /*updating user credits start*/
                        $new_user_balance = $user_balance + $package_bid;
                        $bid_update = $wpdb->update(
                            'tb_5550121_usermeta',
                            array(
                                'meta_value' => $new_user_balance
                            ),
                            array(
                                'user_id' => $user_id,
                        ...
                        if ($bid_update) {
                            ...
                            $result = "Transaction Successful, bids were added to your account!";
                            /*Send Email start*/
                            tbid_bid_package_purchase_user($user_id, $package_bid);
//                        tbid_bid_package_purchase_admin($user_id,$package_bid);
                            /*Send Email end*/
                        } else {
                            /*Bid added trigger start*/
                            $bid_added_trigger = $wpdb->update(
                            ...
                            /*Bid added trigger end*/
                            $result = "Transaction Successful, BUT there was a problem adding the bids to your account.<br>Please <a href='/contact-us'>contact</a> us here";
                        }
                    }
                    break;
                case "?" :
            ...
            if ($result) {
                ...
                <div class="payment_result">
                    ...
                    if ($case == "0") {
                        ...
                        <p class="again">Click <a href="/">here</a> to start bidding</p>
                        <?php
                    } else {
                        ?>
